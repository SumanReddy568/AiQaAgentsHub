...
function storeAuthData(token, email, hash, name = "", userId = null) {
...
async function signup(email, password, name) {
  try {
    ...
    if (!response.ok) {
      ...
      const errorText =
        data.error?.message || data.error || data.message || "Signup failed.";
...
async function login(email, password) {
  try {
    ...
    if (!response.ok) {
      if (Array.isArray(data) && data.length > 0) data = data[0];
      const errorText =
        data.error?.message || data.error || data.message || "Login failed.";
      throw new Error(errorText);
    }
...
async function validateSession() {
  try {
    ...
    const response = await fetch(
      `${AUTH_CONFIG.API_BASE_URL}/me?source=${AUTH_CONFIG.SOURCE}`,
      {
        method: "GET",
