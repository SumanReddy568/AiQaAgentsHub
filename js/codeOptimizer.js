...
function renderOptimizedResult(markdown) {
  ...
  if (!markdown) {
    outputDiv.innerHTML =
      '<div class="text-slate-400 text-center py-8">Paste your code and click "Optimize Code" to generate optimized code</div>';
  ...
  let codeHtml = "";
  if (codeMatch) {
    codeHtml = `<pre class="bg-slate-900 rounded-xl p-4 overflow-x-auto text-sm mb-4"><code>${codeMatch[2]
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")}</code></pre>`;
  ...
  if (explanation) {
    if (typeof marked !== "undefined") {
      explanationHtml = `<div class="prose prose-sm max-w-none mt-2">${marked.parse(
        explanation
      )}</div>`;
    } else {
      console.warn(
        "marked.js library not found. Please include it in your HTML to render the explanation correctly. Falling back to pre-formatted text."
      );
      explanationHtml = `<div class="prose prose-sm max-w-none mt-2"><pre>${explanation
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")}</pre></div>`;
    }
  }
